...
export default function BannersManager() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (editId) {
        const { error } = await supabase.from("banners" as any).update({ ...form, updated_at: new Date().toISOString() }).eq("id", editId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("banners" as any).insert({ ...form, sort_order: banners.length });
        if (error) throw error;
      }
    },
  ...
  const remove = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("banners" as any).delete().eq("id", id); if (error) throw error; },
  ...
  const toggleActive = useMutation({
    mutationFn: async ({ id, is_active }: { id: string; is_active: boolean }) => {
      const { error } = await supabase.from("banners" as any).update({ is_active, updated_at: new Date().toISOString() }).eq("id", id);
      if (error) throw error;
    },
